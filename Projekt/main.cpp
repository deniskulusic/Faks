...
/*Screen click change text
void MyMainWindow::mousePressEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        MyDialog editDialog;
        int ret = editDialog.exec();
        if( ret == QDialog::Accepted ){
            MyLabel->setText(editDialog.lineEdit->text());
        }
    }
}
*/
...
void MyMainWindow::paintEvent(QPaintEvent*){
    ...
    painter.drawLine(MyLabel->pos().x() , MyLabel->pos().y() + MyLabel->height(), MyLabel->pos().x()+MyLabel->width(), MyLabel->pos().y()+MyLabel->height());
}
void MyMainWindow::setStatusBar(){
    statusBar()->showMessage("Size of string: "  + QString::number(MyLabel->text().length()));
}
